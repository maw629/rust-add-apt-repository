...
impl Repository {
    ...
    pub fn add_binary_entry(&mut self, uri: String, dist: String, components: Vec<String>) {
    ...
    pub fn add_source_entry(&mut self, uri: String, dist: String, components: Vec<String>) {
    ...
    pub fn add_both_entries(&mut self, uri: String, dist: String, components: Vec<String>) {
...
pub fn parse_sourceslist_line(line: &str) -> Result<Repository> {
    ...
    if parts.len() < 3 {
        return Err(AppError::InvalidInput(
            "Invalid sources.list line format. Expected: deb <uri> <dist> [component...]"
                .to_string(),
        ));
    }

    let entry_type_str = parts[0];
    let entry_type = entry_type_str.parse::<SourceType>().map_err(|_| {
        AppError::InvalidInput(format!(
            "Invalid entry type: {}. Must be 'deb' or 'deb-src'",
            entry_type_str
        ))
    })?;
        ...
        && !uri.starts_with("file://")
    {
        return Err(AppError::InvalidInput(format!(
            "Invalid URI scheme: {}. Must be http://, https://, ftp://, or file://",
            uri
        )));
    }
...
pub fn parse_uri_shortcut(
    ...
    {
        return Err(AppError::InvalidInput(format!(
            "Invalid URI: {}. Must start with http://, https://, ftp://, or file://",
            uri
        )));
    }
...
mod tests {
    ...
    fn test_parse_sourceslist_line_multiple_components() {
        ...
        assert_eq!(
            repo.entries[0].components,
            vec!["main", "restricted", "universe"]
        );
    }
    ...
    fn test_parse_uri_shortcut_basic() {
        let repo =
            parse_uri_shortcut("http://example.com/ubuntu", Some("noble"), &[], false).unwrap();
    ...
    fn test_parse_uri_shortcut_with_source() {
        let repo =
            parse_uri_shortcut("http://example.com/ubuntu", Some("noble"), &[], true).unwrap();
    ...
    fn test_parse_uri_shortcut_with_components() {
        ...
        let repo =
            parse_uri_shortcut("http://example.com/ubuntu", Some("noble"), &comps, false).unwrap();
