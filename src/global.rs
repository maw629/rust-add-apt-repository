...
pub fn disable_component(component: &str, dry_run: bool) -> Result<()> {
    ...
    for line in content.lines() {
        if let Some(entry) = SourceEntry::from_line(line, main_file.to_path_buf()) {
            ...
            let new_components: Vec<String> = entry
                .components
                .iter()
    ...
    println!(
        "Removing component '{}' from {} repositories:",
        component,
        affected_repos.len()
    );
    for repo in &affected_repos {
...
pub fn enable_pocket(pocket: &str, dry_run: bool) -> Result<()> {
    ...
    if added_count == 0 {
        println!(
            "Pocket '{}' entries already exist for all applicable repositories",
            pocket
        );
        return Ok(());
    }
    ...
    println!(
        "Adding '{}' pocket for {} repositories:",
        pocket, added_count
    );
...
pub fn disable_pocket(pocket: &str, dry_run: bool) -> Result<()> {
    ...
    println!(
        "Disabling '{}' pocket entries ({} repositories):",
        pocket, removed_count
    );
...
pub fn enable_source_code(add_missing: bool, dry_run: bool) -> Result<()> {
    ...
    if add_missing {
        ...
        for (file_path, deb_entry) in deb_entries {
            let key = (
                deb_entry.uri.clone(),
                deb_entry.dist.clone(),
                deb_entry.components.clone(),
            );
    ...
    if !dry_run {
        for file_path in files_to_update {
            let file_entries: Vec<_> = sources
                .entries
                .iter()
